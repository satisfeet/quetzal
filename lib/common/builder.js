    ...
    for (var name in bundles) {
      ...
      if (path.extname(this.path) === '.js') {
        ...
        if (!Object.keys(bundles).indexOf(name)) {
          ...
          script += 'require("' + bundles.common[0].canonical + '");\n';
          script += 'require("' + bundle[0].canonical + '");\n';
        }
      ...
      if (path.extname(this.path) === '.css') {
        ...
        yield writeFile(util.format(options.paths.scripts, name), style);
      }
    }
  }
